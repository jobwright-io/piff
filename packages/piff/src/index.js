...
export class PiffSession {
    ...
    async isEqual(runOptions) {
        ...
        if (this.equality === undefined) {
            throwIfAborted(runOptions?.signal);
            let equality;
            equality = runWithNativeCancellation(runOptions?.signal, (cancellationToken) => loadNativeBinding().isEqual(asBuffer(this.before), asBuffer(this.after), toNativeOptions(this.options), toNativeProgress(runOptions), runOptions?.signal, cancellationToken)).catch((error) => {
...
export async function piff(before, after, options, runOptions) {
...
async function compareNative(before, after, options, runOptions) {
    const raw = await runWithNativeCancellation(runOptions?.signal, (cancellationToken) => loadNativeBinding().piff(asBuffer(before), asBuffer(after), toNativeOptions(options), toNativeProgress(runOptions), runOptions?.signal, cancellationToken));
