...
import type {
  ...
  PdfTextDiffHunk,
  PdfTextDiffLine,
} from '@pdf-differ/core'

export interface PdfDiffPreviewRequest {
  view: PdfPagePreviewView
}

export type PdfDiffPreview = Uint8Array | Blob | string

export type PdfDiffPreviewLoader = (
  pageIndex: number,
  request: PdfDiffPreviewRequest,
) => Promise<PdfDiffPreview>

export interface PdfDiffViewerProps {
  result: PdfDiffResult
  loadPreview: PdfDiffPreviewLoader
  initialPage?: number
  initialView?: PdfPagePreviewView
  initialMode?: PdfDiffViewerMode
  title?: string
  className?: string
}

export type PdfDiffViewerMode = 'review' | 'visual' | 'text'
...
export interface PdfReviewChange {
  ...
  beforeText?: string
  afterText?: string
  beforeBounds?: PdfDiffBounds
  afterBounds?: PdfDiffBounds
...
export function PdfDiffViewer({
  ...
  const [mode, setMode] = useState<PdfDiffViewerMode>(initialMode)
  ...
  return (
    <section
      className={joinClasses('pdf-diff-viewer', className)}
      data-pdf-diff-viewer="true"
      aria-label="PDF comparison viewer"
    >
      <header className="pdf-diff-viewer__header">
        <div>
          <div className="pdf-diff-viewer__kicker">
            <span>PDF / DIFF REGISTER</span>
            <span className="pdf-diff-viewer__kicker-rule" aria-hidden="true" />
            <span>{result.equal ? 'CLEAN' : 'MARKS DETECTED'}</span>
          </div>
          <h2>{title}</h2>
        </div>
        <div className={joinClasses('pdf-diff-viewer__verdict', result.equal ? 'is-clean' : 'is-changed')}>
          <span className="pdf-diff-viewer__verdict-dot" aria-hidden="true" />
          <span>{result.equal ? 'No visual changes' : `${changedPages.length} page${changedPages.length === 1 ? '' : 's'} changed`}</span>
        </div>
      </header>

      <div className="pdf-diff-viewer__toolbar">
        <div className="pdf-diff-viewer__toolbar-controls">
          <div className="pdf-diff-viewer__view-switcher" role="tablist" aria-label="Comparison mode">
            <button
              className={joinClasses('pdf-diff-viewer__view-button', mode === 'review' && 'is-active')}
              type="button"
              role="tab"
            ...
            <button
              className={joinClasses('pdf-diff-viewer__view-button', mode === 'text' && 'is-active')}
              type="button"
              role="tab"
            ...
            <button
              className={joinClasses('pdf-diff-viewer__view-button', mode === 'visual' && 'is-active')}
              type="button"
              role="tab"
          ...
          {mode === 'visual' ? (
            <div className="pdf-diff-viewer__view-switcher" role="tablist" aria-label="Preview view">
              {(Object.keys(VIEW_LABELS) as PdfPagePreviewView[]).map((option) => (
                <button
                  key={option}
                  className={joinClasses('pdf-diff-viewer__view-button', view === option && 'is-active')}
                  type="button"
                  role="tab"
        ...
        <div className="pdf-diff-viewer__toolbar-meta">
          {mode === 'review' ? (
            <>
      ...
      <div className="pdf-diff-viewer__layout">
        <nav className="pdf-diff-viewer__rail" aria-label="Pages">
          <div className="pdf-diff-viewer__rail-label">PAGES</div>
          <div className="pdf-diff-viewer__rail-list">
        ...
        <main className="pdf-diff-viewer__stage">
          {mode === 'review' ? (
            <ReviewDocument
              page={selected}
              index={selectedPage}
            ...
            <div className="pdf-diff-viewer__empty">
              <span className="pdf-diff-viewer__empty-mark" aria-hidden="true">∅</span>
              <strong>No pages to compare</strong>
              <span>The two documents contain no renderable pages.</span>
            </div>
          ) : (
            <div className="pdf-diff-viewer__page-stack">
              {result.pages.map((page, index) => (
                <PageCard
                  key={`${page.beforePage ?? 'x'}-${page.afterPage ?? 'x'}-${index}`}
                  page={page}
        ...
        <aside className="pdf-diff-viewer__inspector" aria-label="Selected page details">
          {mode === 'review' && selected ? (
            ...
            <div className="pdf-diff-viewer__inspector-empty">Select a page to inspect its marks.</div>
          )}
        </aside>
      </div>
    </section>
...
function PageRailItem({ page, index, selected, operationCount, onSelect, onKeyDown }: PageRailItemProps) {
  return (
    <button
      className={joinClasses('pdf-diff-viewer__rail-item', selected && 'is-selected', `is-${page.status}`)}
      type="button"
      ...
      onKeyDown={onKeyDown}
    >
      <span className="pdf-diff-viewer__rail-index">{String(index + 1).padStart(2, '0')}</span>
      <span className="pdf-diff-viewer__rail-status">{statusLabel(page.status)}</span>
      <span className="pdf-diff-viewer__rail-ratio">{operationCount > 0 ? `${operationCount} operation${operationCount === 1 ? '' : 's'}` : page.status === 'equal' ? 'unchanged' : formatRatio(page.changedRatio)}</span>
    </button>
  )
}
...
function ReviewDocument({
  ...
  if (page === undefined) {
    return <div className="pdf-diff-viewer__empty"><strong>No pages to compare</strong><span>The two documents contain no renderable pages.</span></div>
  }
  ...
  return (
    <div className="pdf-diff-viewer__review-document">
      <div className="pdf-diff-viewer__review-topline">
        <div>
          <span className="pdf-diff-viewer__review-eyebrow">PAGE {String(index + 1).padStart(2, '0')}</span>
        ...
        <div className="pdf-diff-viewer__review-actions">
          <button
            className={joinClasses('pdf-diff-viewer__focus-toggle', focusMode && 'is-active')}
            type="button"
          ...
          <div className="pdf-diff-viewer__change-nav" aria-label="Review change navigation">
      ...
      <div ref={canvasRef} className="pdf-diff-viewer__review-canvas" data-status={page.status} data-focus-mode={focusMode ? 'true' : 'false'}>
        <ReviewPagePane
          page={page}
          side="before"
        ...
        />
        <div className="pdf-diff-viewer__review-divider" aria-hidden="true">
          <span>COMPARE</span>
        </div>
        <ReviewPagePane
          page={page}
      ...
      <div className="pdf-diff-viewer__review-caption">
...
interface ReviewPagePaneProps {
  ...
  changes: PdfReviewChange[]
  selectedChange: PdfReviewChange | undefined
  focusBounds: PdfDiffBounds | undefined
...
function ReviewPagePane({ page, side, preview, changes, selectedChange, focusBounds, paperRef, onSelectChange }: ReviewPagePaneProps) {
  ...
  return (
    <div className={joinClasses('pdf-diff-viewer__review-pane', `is-${side}`, !hasPage && 'is-empty')}>
      <div className="pdf-diff-viewer__review-pane-head">
      ...
      <div
        ref={paperRef}
        className={joinClasses('pdf-diff-viewer__review-paper', focusBounds !== undefined && 'is-focused')}
      ...
      >
        {preview.status === 'ready' && preview.source && hasPage ? (
          <img className="pdf-diff-viewer__review-image" style={focusImageStyle(geometry, focusBounds)} src={preview.source} alt={`${side} page ${pageNumber + 1}`} />
        ) : (
          <div className={joinClasses('pdf-diff-viewer__review-placeholder', `is-${preview.status}`)}>
        ...
        <div className="pdf-diff-viewer__review-markers" aria-label={`${side} change markers`}>
          {visibleChanges.map((change, markerIndex) => {
            ...
            return (
              <button
                key={`${side}-${change.id}`}
                className={joinClasses('pdf-diff-viewer__review-marker', `is-${change.kind}`, selected && 'is-selected')}
                type="button"
...
function ReviewRelationshipLayer({ layer }: { layer: ReviewRelationshipLayer | undefined }) {
  ...
  return (
    <svg
      className="pdf-diff-viewer__review-links"
...
function ReviewInspector({
  ...
  return (
    <div className="pdf-diff-viewer__inspector-content pdf-diff-viewer__review-inspector">
      <div className="pdf-diff-viewer__inspector-heading">
        <span className="pdf-diff-viewer__inspector-kicker">PAGE {String(index + 1).padStart(2, '0')}</span>
        <strong>{statusLabel(page.status)}</strong>
      </div>
      <div className="pdf-diff-viewer__review-inspector-count">
        <span>REVIEW OPERATIONS</span>
        <strong>{operations.length}</strong>
        {changes.length !== operations.length ? <small>{changes.length} linked marks</small> : null}
      </div>
      {selectedChange ? (
        <section className="pdf-diff-viewer__selected-change" aria-live="polite">
          <div className="pdf-diff-viewer__selected-change-meta">
          ...
          {selectedOperation && selectedOperation.changes.length > 1 ? (
            <p className="pdf-diff-viewer__operation-context">{selectedChange.label} · linked mark {selectedOperation.changes.findIndex((change) => change.id === selectedChange.id) + 1} of {selectedOperation.changes.length}</p>
          ) : null}
          {selectedChange.kind === 'text' && selectedChange.textHunks?.length ? (
            <div className="pdf-diff-viewer__local-text-diff">
              <div className="pdf-diff-viewer__local-text-diff-heading"><span>LOCAL GIT HUNK</span><span>{selectedChange.textHunks.length} hunk{selectedChange.textHunks.length === 1 ? '' : 's'}</span></div>
              {selectedChange.textHunks.map((hunk, hunkIndex) => <TextDiffHunkView key={`${hunk.beforeStart}-${hunk.afterStart}-${hunkIndex}`} hunk={hunk} />)}
            </div>
          ) : null}
          <div className="pdf-diff-viewer__evidence-pair">
          ...
          {selectedChange.kind === 'text' ? (
            <button className="pdf-diff-viewer__detail-button" type="button" onClick={onOpenText}>Open text details</button>
          ) : null}
        </section>
      ) : (
        <p className="pdf-diff-viewer__regions-empty">This page has no reportable changes. Select another page or use Pixel proof for raster detail.</p>
      )}
      <dl className="pdf-diff-viewer__facts">
      ...
      {changes.length > 0 ? (
        <div className="pdf-diff-viewer__change-list">
          <div className="pdf-diff-viewer__regions-heading"><span>ON THIS PAGE</span><span>{operations.length}</span></div>
          {operations.map((operation, operationIndex) => (
            <button
              key={operation.id}
              className={joinClasses('pdf-diff-viewer__change-list-item', selectedOperation?.id === operation.id && 'is-selected')}
              type="button"
...
function PageWarningNotice({ warnings }: { warnings: PdfPageWarning[] }) {
  ...
  return (
    <section className="pdf-diff-viewer__warning-notice" role="note" aria-label="Evidence cautions">
      <div className="pdf-diff-viewer__warning-heading">EVIDENCE CAUTIONS</div>
      <ul>
...
function PageCard({ page, index, view, selected, loadPreview, onSelect }: PageCardProps) {
  ...
  return (
    <article
      ref={nearViewportRef}
      className={joinClasses('pdf-diff-page', selected && 'is-selected', `is-${page.status}`)}
      style={pageStyle}
      onClick={onSelect}
    >
      <div className="pdf-diff-page__header">
        <span className="pdf-diff-page__number">PAGE {String(index + 1).padStart(2, '0')}</span>
        <span className="pdf-diff-page__status">{statusLabel(page.status)}</span>
        <span className="pdf-diff-page__ratio">{formatRatio(page.changedRatio)} changed</span>
      </div>
      <div className="pdf-diff-page__paper">
        {preview.status === 'ready' && preview.source ? (
          <img
            className="pdf-diff-page__image"
            src={preview.source}
            alt={`${VIEW_LABELS[view]} preview for page ${index + 1}`}
            loading="lazy"
          />
        ) : (
          <div className={joinClasses('pdf-diff-page__placeholder', `is-${preview.status}`)}>
            <span className="pdf-diff-page__placeholder-mark" aria-hidden="true">
...
function PageInspector({ page, index }: { page: PdfPageDiff; index: number }) {
  return (
    <div className="pdf-diff-viewer__inspector-content">
      <div className="pdf-diff-viewer__inspector-heading">
        <span className="pdf-diff-viewer__inspector-kicker">SELECTED PAGE</span>
      ...
      <PageWarningNotice warnings={page.warnings} />
      <div className="pdf-diff-viewer__regions">
        <div className="pdf-diff-viewer__regions-heading">
          <span>REGIONS</span>
          <span>{page.regions.length}</span>
        </div>
        {page.regions.length === 0 ? (
          <p className="pdf-diff-viewer__regions-empty">No reportable regions on this page.</p>
        ) : (
          <ol>
        ...
        {page.regions.length > 24 ? (
          <p className="pdf-diff-viewer__regions-more">+ {page.regions.length - 24} more regions</p>
        ) : null}
      </div>
      <div className="pdf-diff-viewer__semantic">
  ...
  if (semantic === undefined) {
    return (
      <>
        ...
        <p className="pdf-diff-viewer__regions-empty">Run semantic mode to inspect document text changes.</p>
      </>
    )
  }
  ...
  if (textDiff !== undefined && textDiff.hunks.length > 0) {
    return (
      <>
        ...
        <div className="pdf-diff-viewer__text-diff" aria-label="Git-like text diff">
          {textDiff.hunks.map((hunk, hunkIndex) => (
            <div className="pdf-diff-viewer__text-diff-hunk" key={`${hunk.beforeStart}-${hunk.afterStart}-${hunkIndex}`}>
              <div className="pdf-diff-viewer__text-diff-header">
                {formatTextDiffHunk(hunk)}
              </div>
              <div className="pdf-diff-viewer__text-diff-lines">
                {hunk.lines.map((line, lineIndex) => (
                  <div className={joinClasses('pdf-diff-viewer__text-diff-line', `is-${line.kind}`)} key={`${line.kind}-${line.beforeLine ?? 'x'}-${line.afterLine ?? 'x'}-${lineIndex}`}>
                    <span className="pdf-diff-viewer__text-diff-number">{line.beforeLine ?? ''}</span>
                    <span className="pdf-diff-viewer__text-diff-number">{line.afterLine ?? ''}</span>
                    <span className="pdf-diff-viewer__text-diff-prefix" aria-hidden="true">{linePrefix(line.kind)}</span>
                    <span className="pdf-diff-viewer__text-diff-copy">
  ...
  return (
    <>
      ...
      {semantic.changes.length === 0 ? (
        <>
          <TextExtractionNotice semantic={semantic} />
          <p className="pdf-diff-viewer__regions-empty">
        ...
        <ol className="pdf-diff-viewer__semantic-list">
          {semantic.changes.slice(0, 16).map((change) => (
            <li key={change.id} className={`is-${change.kind}`}>
              <span className="pdf-diff-viewer__semantic-kind">{change.kind}</span>
              <div className="pdf-diff-viewer__semantic-copy">
      ...
      {semantic.changes.length > 16 ? (
        <p className="pdf-diff-viewer__regions-more">+ {semantic.changes.length - 16} more text changes</p>
      ) : null}
...
function TextExtractionNotice({
  ...
  return <p className="pdf-diff-viewer__text-notice">{messages.join(' ')} Review the page for authoritative visual evidence.</p>
}
...
function PdfTextDiffDocument({
  ...
  return (
    <div className="pdf-diff-viewer__text-document" data-pdf-diff-text="true">
      <header className="pdf-diff-viewer__text-document-head">
        <div>
          <div className="pdf-diff-viewer__text-document-kicker">DOCUMENT / UNIFIED DIFF</div>
          <h3>What changed in the document.</h3>
        </div>
        <div className="pdf-diff-viewer__text-document-summary">
          <strong>{textPages.length} text page{textPages.length === 1 ? '' : 's'}</strong>
          <span>{hunkCount} hunk{hunkCount === 1 ? '' : 's'} · inline word marks</span>
        </div>
      </header>
      <div className="pdf-diff-viewer__text-file-strip">
        <strong>BEFORE DOCUMENT</strong>
        <span aria-hidden="true">→</span>
        <strong>AFTER DOCUMENT</strong>
      </div>
      <div className="pdf-diff-viewer__text-legend">
        <span className="is-removed">removed</span>
        <span className="is-added">added</span>
        <span>context retained</span>
      </div>
      {textPages.length === 0 ? (
        <div className="pdf-diff-viewer__text-empty">
        ...
        textPages.map(({ page, index, textDiff }) => (
          <section className="pdf-diff-viewer__text-page" key={index}>
            <button className="pdf-diff-viewer__text-page-heading" type="button" onClick={() => onSelectPage(index)}>
              <strong>PAGE {String(index + 1).padStart(2, '0')}</strong>
              <span>{statusLabel(page.status)}</span>
              <span>{textDiff.hunks.length} hunk{textDiff.hunks.length === 1 ? '' : 's'}</span>
            </button>
            <div className="pdf-diff-viewer__text-hunks">
              {textDiff.hunks.map((hunk, hunkIndex) => (
      ...
      {visualOnlyPages.length > 0 ? (
        <div className="pdf-diff-viewer__text-foot">
...
function TextDiffHunkView({ hunk }: { hunk: PdfTextDiffHunk }) {
  ...
  return (
    <div className="pdf-diff-viewer__text-hunk">
      <div className="pdf-diff-viewer__text-hunk-header">
        {`@@ -${hunk.beforeStart},${beforeCount} +${hunk.afterStart},${afterCount} @@`}
      </div>
      <div className="pdf-diff-viewer__text-lines">
        {hunk.lines.map((line, lineIndex) => (
...
function TextDiffLineView({ line }: { line: PdfTextDiffLine }) {
  return (
    <div className={joinClasses('pdf-diff-viewer__text-line', `is-${line.kind}`)}>
      <span className="pdf-diff-viewer__text-line-number">{line.beforeLine ?? ''}</span>
      <span className="pdf-diff-viewer__text-line-number">{line.afterLine ?? ''}</span>
      <span className="pdf-diff-viewer__text-line-prefix" aria-hidden="true">{linePrefix(line.kind)}</span>
      <span className="pdf-diff-viewer__text-line-copy">
        {line.spans.map((span, spanIndex) => (
          <span className={`is-${span.kind}`} key={`${span.kind}-${spanIndex}`}>{span.text}</span>
        ))}
      </span>
...
function groupSemanticChanges(changes: PdfSemanticTextChange[], textDiff?: NonNullable<PdfPageDiff['semantic']>['textDiff']): PdfReviewChange[] {
  ...
  const groups: Array<{ changes: PdfSemanticTextChange[]; bounds?: PdfDiffBounds; lineBounds?: PdfDiffBounds }> = []
...
function clusterVisualRegions(
  regions: PdfDiffResult['pages'][number]['regions'],
  renderWidth: number,
  renderHeight: number,
...
function boundsStyle(bounds: PdfDiffBounds, geometry: PdfPageGeometry): CSSProperties {
  return {
...
function offsetBounds(bounds: PdfDiffBounds, offsetX: number, offsetY: number): PdfDiffBounds {
  return {
    x: bounds.x + offsetX,
    y: bounds.y + offsetY,
    width: bounds.width,
...
function focusCropFor(bounds: PdfDiffBounds | undefined, geometry: PdfPageGeometry): PdfDiffBounds | undefined {
...
function focusImageStyle(geometry: PdfPageGeometry, focusBounds: PdfDiffBounds | undefined): CSSProperties {
...
function unionMany(bounds: Array<PdfDiffBounds | undefined>): PdfDiffBounds | undefined {
  return bounds.reduce<PdfDiffBounds | undefined>((current, next) => unionBounds(current, next), undefined)
}

function reviewAnchorBounds(change: PdfSemanticTextChange): PdfDiffBounds | undefined {
...
function reviewSideBounds(change: PdfSemanticTextChange, side: 'before' | 'after'): PdfDiffBounds | undefined {
...
function unionBounds(left: PdfDiffBounds | undefined, right: PdfDiffBounds | undefined): PdfDiffBounds | undefined {
...
function sameTextLine(left: PdfDiffBounds, right: PdfDiffBounds): boolean {
...
function nearbyBounds(left: PdfDiffBounds, right: PdfDiffBounds, gap: number): boolean {
...
function countTextPages(result: PdfDiffResult): number {
  return result.pages.filter((page) => (page.semantic?.textDiff?.hunks.length ?? 0) > 0).length
}

function countTextHunks(result: PdfDiffResult): number {
