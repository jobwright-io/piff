...
React presentation for the serializable `@pdf-differ/core` result. The viewer keeps the native layer out of React: callers provide a preview loader, and page images are requested only as cards approach the viewport.

```tsx
import { PdfDiffSession } from '@pdf-differ/core'
import { PdfDiffViewer } from '@pdf-differ/react'
import '@pdf-differ/react/style.css'

const session = await PdfDiffSession.open(beforePdf, afterPdf, { dpi: 144 })
