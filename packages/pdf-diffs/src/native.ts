...
export function loadNativeBinding(): NativeBinding {
  ...
  const override = process.env.PDF_DIFF_NATIVE_MODULE
  const localModule = resolve(
    projectRoot,
    'artifacts/pdf-diff.linux-x64-gnu.node',
  )
  const packageNames = [
    runtimePackageName(),
    '@pdf-differ/core-linux-x64-gnu',
    '@pdf-differ/core-linux-x64-musl',
    '@pdf-differ/core-linux-arm64-gnu',
    '@pdf-differ/core-linux-arm64-musl',
    '@pdf-differ/core-darwin-x64',
    '@pdf-differ/core-darwin-arm64',
    '@pdf-differ/core-win32-x64-msvc',
  ...
  throw new Error(
    `Could not load the pdf-differ native module for ${runtimeTargetLabel()}. Set PDF_DIFF_NATIVE_MODULE or install a platform package.`,
  )
}

function runtimePackageName(): string | undefined {
  const target = runtimeTarget()
  return target === undefined ? undefined : `@pdf-differ/core-${target}`
}
...
export function toNativeOptions(
  options: PdfDiffOptions | undefined,
...
export function toNativeProgress(
  options: PdfDiffRunOptions | undefined,
...
function isProgressPhase(phase: string): phase is PdfDiffProgressPhase {
  return (
    phase === 'loading' ||
