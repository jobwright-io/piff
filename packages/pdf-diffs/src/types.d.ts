...
export type PdfDiffProgressPhase = 'loading' | 'fingerprinting' | 'rendering' | 'comparing';
export interface PdfDiffProgress {
    phase: PdfDiffProgressPhase;
    completed: number;
    total: number;
}
export interface PdfDiffRunOptions {
    signal?: AbortSignal;
    onProgress?: (event: PdfDiffProgress) => void;
}
...
export interface PdfDiffBounds {
    x: number;
    y: number;
    width: number;
    height: number;
