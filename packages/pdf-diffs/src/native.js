...
export function loadNativeBinding() {
    ...
    const localModule = resolve(projectRoot, 'artifacts/pdf-diff.linux-x64-gnu.node');
    const packageNames = [
        runtimePackageName(),
    ...
    throw new Error(`Could not load the pdf-differ native module for ${runtimeTargetLabel()}. Set PDF_DIFF_NATIVE_MODULE or install a platform package.`);
}
