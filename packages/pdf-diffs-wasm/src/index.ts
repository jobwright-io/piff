...
type RawTextBlockDiff = {
  id: string
  kind: NonNullable<PdfDiffResult['pages'][number]['semantic']>['blocks'][number]['kind']
...
export interface PdfDiffWasmModule {
...
export interface PdfDiffWasmRuntimeOptions {
  /** The wasm-bindgen instance object passed to pdfium-render's initializer. */
  localModule?: unknown
}

/**
 * Synchronous calls into a browser WASM module. Use PdfDiffWasmWorkerClient when these calls
 * must not block the browser event loop.
 */
export class PdfDiffWasmRuntime {
  constructor(
    private readonly module: PdfDiffWasmModule,
    private readonly runtimeOptions: PdfDiffWasmRuntimeOptions = {},
  ) {}
  ...
  diffPdf(
    ...
    const raw = resolveExport<DiffPdf>(this.module, 'diff_pdf', 'diffPdf')(
      before,
      after,
  ...
  isEqualPdf(
    ...
    return resolveExport<EqualPdf>(this.module, 'is_equal_pdf', 'isEqualPdf')(
      before,
      after,
      serializeOptions(options),
    )
...
export interface PdfDiffWasmWorkerPort {
...
export type PdfDiffWasmWorkerRequest =
  | {
      id: number
      type: 'diff'
      before: ArrayBuffer
...
export type PdfDiffWasmWorkerResponse =
  | { id: number; ok: true; type: 'diff'; result: PdfDiffResult }
...
type WorkerOperation = Exclude<PdfDiffWasmWorkerRequest, { type: 'cancel' }>
...
export class PdfDiffWasmWorkerClient {
  private nextId = 1
  private closed = false
  private readonly pending = new Map<number, PendingRequest>()
  private readonly onMessage = (event: MessageEvent<PdfDiffWasmWorkerResponse>): void => {
  ...
  constructor(private readonly worker: PdfDiffWasmWorkerPort) {
  ...
  private request<T>(
    ...
    const message = { ...request, id } as PdfDiffWasmWorkerRequest
    return new Promise<T>((resolve, reject) => {
      const onAbort = (): void => {
        this.pending.delete(id)
        this.worker.postMessage({ id, type: 'cancel' } satisfies PdfDiffWasmWorkerRequest)
...
export function createPdfDiffWasmWorkerHandler(
  runtime: PdfDiffWasmRuntime,
  scope: { postMessage(message: unknown, transfer?: Transferable[]): void },
): (event: MessageEvent<PdfDiffWasmWorkerRequest>) => void {
...
async function handleWorkerRequest(
  runtime: PdfDiffWasmRuntime,
  scope: { postMessage(message: unknown, transfer?: Transferable[]): void },
  request: Exclude<PdfDiffWasmWorkerRequest, { type: 'cancel' }>,
  ...
  try {
    ...
    if (request.type === 'diff') {
      const result = runtime.diffPdf(before, after, request.options)
      if (!cancelled.has(request.id)) {
        scope.postMessage({ id: request.id, ok: true, type: 'diff', result })
      }
    } else if (request.type === 'equal') {
      const result = runtime.isEqualPdf(before, after, request.options)
      if (!cancelled.has(request.id)) {
    ...
    if (!cancelled.has(request.id)) {
      ...
      } satisfies PdfDiffWasmWorkerResponse)
    }
  } finally {
...
interface WasmResult {
  ...
  pages: Array<{
    ...
    figures?: Array<{
      id: string
      status: PdfDiffResult['pages'][number]['figures'][number]['status']
    ...
    semantic?: {
      ...
      changes: Array<{
        id: string
        kind: NonNullable<PdfDiffResult['pages'][number]['semantic']>['changes'][number]['kind']
        before_text?: string | null
        after_text?: string | null
        before_bounds?: { x: number; y: number; width: number; height: number } | null
        after_bounds?: { x: number; y: number; width: number; height: number } | null
...
function mapResult(raw: WasmResult): PdfDiffResult {
  return {
...
function resolveExport<T extends (...args: any[]) => any>(
  module: PdfDiffWasmModule,
  snakeName: string,
  camelName: string,
): T {
  const candidate = module[snakeName as keyof PdfDiffWasmModule]
    ?? module[camelName as keyof PdfDiffWasmModule]
...
function serializeOptions(options: PdfDiffOptions | undefined): string | undefined {
