...
use pdf_diff_pdfium::{
    ...
    PdfDiffMode, PdfDiffOptions, PdfDiffResult, PdfPasswords, PdfRenderRequest, PdfResourceLimits,
    PreviewView, ProgressEvent, ProgressPhase,
};
use pdf_diff_semantic::TextReadingOrder;
...
fn split_options(options: Option<DiffOptionsJs>) -> napi::Result<(PdfDiffOptions, PasswordsOwned)> {
...
fn pdf_options(options: &DiffOptionsJs) -> napi::Result<PdfDiffOptions> {
    let defaults = PdfDiffOptions::default();
    if options
        .channel_tolerance
    ...
    let mode = match options.mode.as_deref() {
        None | Some("visual") => PdfDiffMode::Visual,
        Some("semantic") => PdfDiffMode::Semantic,
        Some(value) => {
            return Err(Error::from_reason(format!(
