...
use pdf_diff_core::DiffOptions;
use pdf_diff_pdfium::{
    check_pdfium, compare_files_with_passwords, default_linux_pdfium_path,
    is_equal_bytes_with_passwords_and_progress, PageMatching, PageStatus, PdfDiffError,
    PdfDiffMode, PdfDiffOptions, PdfDiffResult, PdfEngineInfo, PdfPasswords, PdfResourceLimits,
    RESULT_SCHEMA_VERSION,
};
use pdf_diff_semantic::{SemanticChangeKind, TextDiffLineKind, TextReadingOrder};
use serde::Serialize;
use thiserror::Error;
...
struct ComparisonSettings {
    ...
    #[arg(
        long,
        default_value_t = pdf_diff_semantic::DEFAULT_TEXT_DIFF_CONTEXT_LINES
    )]
    context_lines: usize,
    /// Shared password for both encrypted PDFs. Prefer PDF_DIFF_PASSWORD for automation.
    #[arg(
        long,
        env = "PDF_DIFF_PASSWORD",
...
impl CliError {
    ...
    fn exit_code(&self) -> i32 {
        match self {
            Self::Engine(PdfDiffError::Cancelled) => EXIT_CANCELLED,
            _ => EXIT_ERROR,
        }
    }
}
...
fn run_equal(command: EqualCommand) -> Result<i32, CliError> {
    ...
    let output = EqualityOutput {
        ...
        engine: pdf_diff_pdfium::ENGINE_INFO,
        equal,
    };
...
impl ComparisonSettings {
    ...
    fn to_options(&self, include_previews: bool, mode: DiffModeArg) -> PdfDiffOptions {
        ...
        PdfDiffOptions {
            ...
            mode: match mode {
                DiffModeArg::Visual => PdfDiffMode::Visual,
                DiffModeArg::Semantic => PdfDiffMode::Semantic,
            },
...
fn render_text_diff(result: &PdfDiffResult, before: &Path, after: &Path) -> String {
...
fn write_review_item_summary(output: &mut String, item: &pdf_diff_pdfium::PdfDocumentReviewItem) {
    match item.kind {
...
fn render_inline_line(line: &pdf_diff_semantic::TextDiffLine) -> String {
    let has_equal = line
        .spans
        .iter()
        .any(|span| span.kind == pdf_diff_semantic::TextDiffSpanKind::Equal);
    let has_change = line.spans.iter().any(|span| {
        matches!(
            span.kind,
            pdf_diff_semantic::TextDiffSpanKind::Added
                | pdf_diff_semantic::TextDiffSpanKind::Removed
        )
    });
    ...
    line.spans
        ...
        .map(|span| {
            ...
            match span.kind {
                pdf_diff_semantic::TextDiffSpanKind::Equal => text,
                pdf_diff_semantic::TextDiffSpanKind::Added => mark_inline_change(&text, "{+", "+}"),
                pdf_diff_semantic::TextDiffSpanKind::Removed => {
                    mark_inline_change(&text, "[-", "-]")
                }
            }
        })
        .collect()
...
fn write_page_summary(output: &mut String, page: &pdf_diff_pdfium::PdfPageDiff) {
    match page.status {
        PageStatus::Inserted => {
