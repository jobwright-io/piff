...
#[derive(Debug, Clone, Copy)]
pub struct PdfDiffOptions {
    pub dpi: f32,
    pub diff: DiffOptions,
    pub page_matching: PageMatching,
    pub mode: PdfDiffMode,
...
impl Default for PdfDiffOptions {
    fn default() -> Self {
        Self {
            dpi: 144.0,
            diff: DiffOptions::default(),
            page_matching: PageMatching::default(),
            mode: PdfDiffMode::default(),
            reading_order: TextReadingOrder::default(),
            text_context_lines: pdf_diff_semantic::DEFAULT_TEXT_DIFF_CONTEXT_LINES,
...
pub struct PdfDocumentTextDiffPage {
    ...
    pub after_page: Option<usize>,
    pub status: PageStatus,
    pub blocks: Vec<pdf_diff_semantic::SemanticTextBlockDiff>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub fn check_pdfium(library_path: Option<&Path>) -> Result<PdfEngineInfo, PdfDiffError> {
...
) -> Result<PdfDiffResult, PdfDiffError> {
...
pub fn is_equal_bytes_with_passwords_and_progress(
    ...
    with_pdfium(library_path, |pdfium| {
        ...
        for (page_index, page_pair) in page_pairs.iter().copied().enumerate() {
            ...
            if matches!(options.mode, PdfDiffMode::Semantic)
...
pub fn render_page_diff_png_with_passwords_and_cancellation(
    ...
    with_pdfium(library_path, |pdfium| {
        ...
        if page_index >= page_pairs.len() {
            return Err(PdfDiffError::PageIndexOutOfBounds {
                page_index,
...
fn validate_options(options: PdfDiffOptions) -> Result<(), PdfDiffError> {
    if options.dpi <= 0.0 || !options.dpi.is_finite() {
        return Err(PdfDiffError::InvalidDpi);
    }
    if options.limits.max_input_bytes == Some(0) {
        return Err(PdfDiffError::InvalidMaxInputBytes);
    }
    if options.limits.max_pages == Some(0) {
        return Err(PdfDiffError::InvalidMaxPages);
    }
    if options.limits.max_page_pixels == Some(0) {
        return Err(PdfDiffError::InvalidMaxPagePixels);
    }
    if options.text_context_lines > MAX_TEXT_DIFF_CONTEXT_LINES {
        return Err(PdfDiffError::InvalidTextContextLines);
    }
    Ok(())
}
...
fn ensure_file_input_limit(
    ...
    let bytes = std::fs::metadata(path)
        .map_err(|error| PdfDiffError::InputMetadata {
            document,
            message: error.to_string(),
        })?
        .len();
    if bytes > max_input_bytes as u64 {
        return Err(PdfDiffError::InputTooLarge {
            document,
...
fn ensure_page_limit(
    ...
    if let Some(max_pages) = max_pages {
        if page_count > max_pages {
            return Err(PdfDiffError::PageLimitExceeded {
                document,
                page_count,
                max_pages,
            });
...
fn ensure_page_pixels(
    ...
    if let Some(max_pixels) = max_page_pixels {
        if pixels > max_pixels {
            return Err(PdfDiffError::PagePixelsExceeded {
                width,
                height,
                pixels,
                max_pixels,
...
fn compare_documents(
    ...
    for (page_index, page_pair) in page_pairs.iter().copied().enumerate() {
        ...
        let semantic = if matches!(options.mode, PdfDiffMode::Semantic) {
    ...
    let text_diff =
        matches!(options.mode, PdfDiffMode::Semantic).then(|| build_document_text_diff(&pages));
    Ok(PdfDiffResult {
...
fn document_review_item(
    page_index: usize,
    page: &PdfPageDiff,
    block: &pdf_diff_semantic::SemanticTextBlockDiff,
...
fn build_page_pairs(
    ...
    progress: Option<&dyn Fn(ProgressEvent)>,
    cancellation: Option<&CancellationToken>,
) -> Result<(Vec<PagePair>, u128), PdfDiffError> {
    ensure_not_cancelled(cancellation)?;
    let before_page_count = before.pages().len().max(0) as usize;
    let after_page_count = after.pages().len().max(0) as usize;
...
fn render_fingerprints(
    ...
    cancellation: Option<&CancellationToken>,
    max_page_pixels: Option<u64>,
) -> Result<Vec<pdf_diff_core::PageFingerprint>, PdfDiffError> {
...
fn ensure_not_cancelled(cancellation: Option<&CancellationToken>) -> Result<(), PdfDiffError> {
    if is_cancelled(cancellation) {
        Err(PdfDiffError::Cancelled)
    } else {
        Ok(())
    }
}
...
) -> Result<SemanticPageDiff, PdfDiffError> {
...
fn extract_text_runs(
    ...
    reading_order: TextReadingOrder,
    cancellation: Option<&CancellationToken>,
) -> Result<(Vec<TextRun>, TextExtractionSummary), PdfDiffError> {
    let page_height = page.height().value;
...
fn with_pdfium<T>(
    library_path: Option<&Path>,
    operation: impl FnOnce(&Pdfium) -> Result<T, PdfDiffError>,
) -> Result<T, PdfDiffError> {
    let state = PDFIUM.get_or_init(|| create_pdfium(library_path).map(Mutex::new));
    let mutex = state
        .as_ref()
        .map_err(|error| PdfDiffError::PdfiumBinding(error.clone()))?;
    let pdfium = mutex
        .lock()
        .map_err(|_| PdfDiffError::PdfiumBinding("Pdfium mutex was poisoned".to_owned()))?;
    operation(&pdfium)
}
...
fn render_page(
    ...
    if target_width_value > i32::MAX as f32 || target_height_value > i32::MAX as f32 {
        return Err(PdfDiffError::PageDimensionsTooLarge);
    }
    ...
    if target_height <= MAX_RENDER_TILE_HEIGHT {
        ...
        return RgbaImage::from_raw(width, height, bitmap.as_rgba_bytes())
            .ok_or(PdfDiffError::ImageConversion);
    }
    ...
    for y_offset in (0..target_height).step_by(tile_height as usize) {
        ...
        if bytes.len() < expected_bytes {
            return Err(PdfDiffError::ImageConversion);
        }
        for row in 0..rows {
            ensure_not_cancelled(cancellation)?;
...
fn encode_png(image: RgbaImage) -> Result<Vec<u8>, PdfDiffError> {
    let mut output = Cursor::new(Vec::new());
    DynamicImage::ImageRgba8(image)
        .write_to(&mut output, ImageFormat::Png)
        .map_err(|error| PdfDiffError::PreviewEncoding(error.to_string()))?;
...
fn load_pdf_from_file<'a>(
    ...
    password: Option<&str>,
    document: &'static str,
) -> Result<PdfDocument<'a>, PdfDiffError> {
    pdfium
...
fn pdfium_load_error(error: PdfiumError, document: &'static str) -> PdfDiffError {
    match error {
        PdfiumError::PdfiumLibraryInternalError(PdfiumInternalError::PasswordError) => {
            PdfDiffError::PasswordRequired { document }
        }
        PdfiumError::PdfiumLibraryInternalError(PdfiumInternalError::SecurityError) => {
            PdfDiffError::SecurityUnsupported { document }
        }
...
fn pdfium_error(error: PdfiumError) -> PdfDiffError {
    PdfDiffError::Pdfium(error.to_string())
}
...
mod tests {
    ...
    fn resource_limits_reject_zero_values() {
        let options = PdfDiffOptions {
            limits: PdfResourceLimits {
                max_input_bytes: Some(0),
                ..PdfResourceLimits::default()
            },
            ..PdfDiffOptions::default()
        };

        assert!(matches!(
            validate_options(options),
            Err(PdfDiffError::InvalidMaxInputBytes)
        ));
    }
    ...
    fn text_diff_context_rejects_unbounded_values() {
        ...
        assert!(matches!(
            validate_options(options),
            Err(PdfDiffError::InvalidTextContextLines)
        ));
    }
    ...
    fn page_pixel_limit_rejects_large_renders() {
        ...
        assert!(matches!(
            error,
            PdfDiffError::PagePixelsExceeded {
                width: 10,
                height: 10,
                pixels: 100,
                max_pixels: 99,
    ...
    fn password_load_errors_have_stable_classifications() {
        assert!(matches!(
            ...
            PdfDiffError::PasswordRequired { document: "before" }
        ));
        assert!(matches!(
            pdfium_load_error(
                PdfiumError::PdfiumLibraryInternalError(PdfiumInternalError::SecurityError),
            ...
            PdfDiffError::SecurityUnsupported { document: "after" }
        ));
    }
}
